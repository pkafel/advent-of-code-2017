fun main(args: Array<String>) {
    println(highEntropyPassphasePart(challengeInput, false))
    println(highEntropyPassphasePart(challengeInput, true))
}

fun highEntropyPassphasePart(input: String, countAnagrams: Boolean): Int {
    return input.split("\n")
            .map { if(countAnagrams) isValidPassphaseWithAnagrams(it) else isValidPassphase(it) }
            .filter { it }
            .size
}

fun isValidPassphase(input: String): Boolean {
    val passphases = input.split(" ")
    return passphases.size == passphases.toSet().size
}

fun isValidPassphaseWithAnagrams(input: String): Boolean {
    val passphases = input.split(" ").map { it.toList().sorted() }
    return passphases.size == passphases.toSet().size
}

val challengeInput = "nyot babgr babgr kqtu kqtu kzshonp ylyk psqk\n" +
